...
process.on('exit', function (code) {
  console.log('Running process.on in cmd.js')
  if (code !== 0) {
    console.log('code!==0 in process.on in cmd.js')
...
if (process.env.DEBUG || argv.stdout) {
  console.log('process.env.DEBUG || argv.stdout in cmd.js')
  argv.quiet = argv.q = true
}

var started = Date.now()
function getRuntime () {
  console.log('Running getRunTime')
...
if (command === 'help' || argv.help) {
  console.log('if command=help in cmd.js')
  runHelp()
} else if (command === 'version' || argv.version) {
  console.log('if command=version incmd.js')
  runVersion()
} else if (command === 'info') {
  console.log('if command=info in cmd.js')
  runInfo(/* torrentId */ argv._[1])
} else if (command === 'create') {
  console.log('if command=create in cmd.js')
  runCreate(/* input */ argv._[1])
} else if (command === 'download') {
  console.log('if command=download in cmd.js')
  runDownload(/* torrentId */ argv._[1])
} else if (command === 'seed') {
  console.log('if command=seed in cmd.js')
  runSeed(/* input */ argv._[1])
} else if (command) {
  // assume command is "download" when not specified
  console.log('assume command is download when not specified')
  runDownload(/* torrentId */ command)
} else {
  console.log('Run help in cmd.js')
  runHelp()
}

function runVersion () {
  console.log('inside the runVersion function in cmd.js')
  console.log(require('../package.json').version)
  process.exit(0)
}

function runHelp () {
  console.log('inside the runHelp function in cmd.js')
...
function runInfo (torrentId) {
  console.log('inside runInfo function in cmd.js')
  var parsedTorrent
  try {
    parsedTorrent = parseTorrent(torrentId)
    console.log('in runInfo: try')
  } catch (err) {
    console.log('in runInfo:catch')
    // If torrent fails to parse, it could be a filesystem path, so don't consider it
    // an error yet.
  }

  if (!parsedTorrent || !parsedTorrent.infoHash) {
    console.log('In runInfo: if !parsedTorrent || !parsedTorrent.infoHash')
    try {
  ...
  if (argv.out) {
    console.log('In runInfo: if argv.out')
...
function runCreate (input) {
  console.log('Inside runCreate function in cmd.js')
...
function runDownload (torrentId) {
  console.log('Inside runDownload function in cmd.js')
  ...
  function remove () {
    console.log('Inside function remove in cmd.js')
  ...
  torrent.on('infoHash', function () {
    console.log('torrent.on(infoHash) in cmd.js')
    function updateMetadata () {
      console.log('Update metdata in torrent.on(infoHash)')
    ...
    if (!argv.quiet) {
      console.log(' if !argv.quiet in torrent.on(infoHash)')
  ...
  torrent.on('verifying', function (data) {
    console.log('torrent.on(verifying)')
  ...
  server.listen(argv.port, function () {
    ...
    if (torrent.ready) onReady()
      console.log('Sever.listen: if: torrent.ready')
    else torrent.once('ready', onReady)
      console.log('Server.listen else: torrent.once')
  }).once('connection', function () {
    serving = true
  })

  function onReady () {
    console.log('Inside function inReady in cmd.js')
    ...
    if (argv.select) {
      console.log(' if argv.select in onReady')
  ...
  function onSelection (index) {
    console.log('Inside onSelection function')
  ...
  function done () {
    console.log('Inside fucntion done')
    if (!serving) process.exit(0)
  }
}

function runSeed (input) {
  console.log('inside function runSeed')
  var client = new WebTorrent({
    blocklist: argv.blocklist
  })
  .on('error', errorAndExit)
...
function drawTorrent (torrent) {
  console.log('inside function drawTorrent')
  if (!argv.quiet) {
  ...
  function draw () {
    ...
    torrent.swarm.wires.every(function (wire) {
      console.log('torrent.swarm.wires.every')
      var progress = '?'
      if (torrent.parsedTorrent) {
        console.log('in torrent.swarm.wires.every if:torrent.parsedTorrent')
        var bits = 0
      ...
      if (wire.peerChoking) tags.push('choked')
        console.log('in torrent.swarm.wires.every  if: wire.peerChoking')
  ...
  function active (wire) {
    console.log('inside function active')
    return !wire.peerChoking
  }
}

function errorAndExit (err) {
  console.log('inside function errorAndExit')
